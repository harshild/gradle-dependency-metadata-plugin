...
class DependencyMetadataPluginTest {
    ...
    private boolean isJSONReportFileExists() {
        return new File(testProjectDir.getRoot().getAbsolutePath() + "/build/reports/dependency-metadata.json").exists()
    }
    ...
    @Test
    void plugin_canExecute_reportGeneratorTask_AndGenerateJSONOutputIfSpecified() throws IOException {
        String compileDep = buildFileContent.concat("\n" +
                "apply plugin: 'java'\n\n"
                + "repositories {\n" +
                "    mavenCentral()\n" +
        ...
        assertEquals(result.task(":" + CommandConstants.GENERATE_DEPENDENCY_METADATA_REPORT).getOutcome(), SUCCESS)
        assertTrue(result.getOutput().contains(MetadataReportGeneratorTask.INFO_MESSAGE))
        assertTrue(isJSONReportFileExists())
    }
